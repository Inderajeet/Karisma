...
(function( $ ) {
	...
	$( document ).ready(function() {
		...
		if( o_stat ){
			...
			var header_offset = happysmile_ajax_var.header_offset;
			var mheader_offset = happysmile_ajax_var.mheader_offset;
			var res_width = happysmile_ajax_var.res_width;
		...
		if( $('.zozo-mc').length ){
			$('.zozo-mc').on( "click", function () {
				...
				if( mc_form.find('input[name="zozo_mc_email"]').val() == '' ){
					mc_wrap.find('.mc-notice-msg').text( happysmile_ajax_var.must_fill );
				}else{
					c_btn.attr( "disabled", "disabled" );
					$.ajax({
						type: "POST",
						url: happysmile_ajax_var.ajax_url,
						data: 'action=zozo-mc&nonce='+happysmile_ajax_var.mc_nounce+'&'+mc_form.serialize(),
	...
	$( window ).load(function() {
		...
		//Happysmile masonry
		if( $(".happysmile-masonry").length ){
			var masonry_on_time;			
			var masonry_cols = $(".happysmile-masonry").data('columns');			
			var masonry_gutter = $(".happysmile-masonry").data('gutter');
			
			$(".happysmile-masonry").rcmasonry({
    ...
    //Elementor RTL issue
    function happysmile_for_elementor_row(){
    ...
    $(window).resize(function(event){
		...
		if( $(".sticky-outer").length ){
			$(".sticky-outer").each(function(){
				...
				_sticky_ele.happysmile_stickypart();
				var sticky_on_time;
				$(window).resize(function(event){
					clearTimeout(sticky_on_time);
					sticky_on_time = setTimeout(function(){ _sticky_ele.happysmile_stickypart(); }, 300);
				});
			});
		}
...
(function ( $ ) {
	...
	//Make sticky
	$.fn.happysmile_stickypart = function( options ){
